...
from bayesinsight.awb_model.models.variablemodels import ExogVariableDetails, ControlVariableDetails, MediaVariableDetails, LocalTrendsVariableDetails, SeasonVariableDetails
from bayesinsight.awb_model.utils import var_dims
from bayesinsight.awb_model.constants import MFFCOLUMNS
...
class BayesInsightModel(BaseModel):
    ...
    def predict(self, data: Optional[dict[str, xr.DataArray]]=None, subset: Optional[dict[str, slice | int | str]]=None, var_names: Optional[list[str]]=None) -> xr.Dataset:
        if subset is None:
            subset = dict()
        if not self.fitted:
            raise ValueError("Model must be fitted first")

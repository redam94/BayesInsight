...
def compute_contributions(
        ...
        vars: List[VariableDetails]) -> xr.Dataset:
...
def non_heirarcical_bayesinsight_loss_function(
    ...
    with pymc_model:
        ...
        for i, media_var in enumerate(vars)})
        if not external_variables is None:
            pass
...
def eq_constraint(x: list, starting_budget: dict[str, float]):
    ...
    updated_budget = sum((1+x[i])*starting_budget[media_var] for i, media_var in enumerate(starting_budget.keys()))
